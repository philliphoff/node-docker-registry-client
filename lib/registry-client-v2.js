            ...
            function _putManifest(_, next) {
                ...
                self._httpapi.put({
                    ...
                    req.on('result', function onReqResult(reqErr, res_) {
                        res = res_;
                        // Ignoring (or rather deferring) the reqErr, as the
                        // Docker Registry will usually provide a more detailed
                        // JSON error message in the response body, so we'll
                        // wait/read the data in order to get that detailed
                        // error msg.
                        res.body = '';
                        ...
                        res.on('end', function onResEnd() {
                            ...
                            var regErr = _getRegistryResponseErr(res,
                                errors.UploadError);
                            digest = res.headers['docker-content-digest'];
                            location = res.headers.location;
                            // Prefer the more detailed regErr over the reqErr.
                            next(regErr || reqErr);
                        });
                    });
